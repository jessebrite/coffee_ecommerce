...
// Set the page title
$page_title = 'Cart - See your shopping list';
...
if (isset($sp_type, $pid, $_GET['action']) && ($_GET['action']) == 'add') {
	$result = $conn->query("CALL add_to_cart('$uid', '$sp_type', $pid, 1)");
	...
	// Add it to the cart
	$result = $conn->query("CALL add_to_cart('$uid', '$sp_type', $pid, $qty)");
	if (!$result) echo $conn->error; // for debugging purposes
	...
	foreach ($_POST['quantity'] as $sku => $qty) {
		...
		if (isset($sp_type, $pid)) {
			...
			$result = $conn->query("CALL update_cart('$uid', '$sp_type', $pid, $qty)");
			if (!$result) echo $conn->error; // for debugging purposes
		}
	} // End of FOREACH loop
} // End of main IF

$result = $conn->query("CALL get_shopping_cart_contents('$uid')");
